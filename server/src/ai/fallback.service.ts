...
export class FallbackService implements AIService {
  async generateResponse(
    messages: ChatMessage[],
    existingInfo: CollectedInfo
  ): Promise<{ reply: string; updatedInfo: CollectedInfo; isComplete: boolean }> {
    const updatedInfo = await this.analyzeConversation(messages);
    const mergedInfo: CollectedInfo = { ...existingInfo, ...updatedInfo };
  ...
  async analyzeConversation(messages: ChatMessage[]): Promise<CollectedInfo> {
    const info: CollectedInfo = {};
    const text = messages.map((m) => m.content).join(' ').toLowerCase();

    // Recipient extraction
    if (text.includes('wife') || text.includes('spouse') || text.includes('partner')) info.recipient = 'Wife';
    else if (text.includes('husband')) info.recipient = 'Husband';
    else if (text.includes('mother') || text.includes('mom')) info.recipient = 'Mother';
    else if (text.includes('father') || text.includes('dad')) info.recipient = 'Father';
    else if (text.includes('friend')) info.recipient = 'Dear Friend';
    else if (text.includes('client') || text.includes('executive')) info.recipient = 'Valued Associate';

    // Occasion extraction
    if (text.includes('anniversary')) info.occasion = 'Anniversary';
    else if (text.includes('birthday')) info.occasion = 'Birthday';
    else if (text.includes('holiday') || text.includes('christmas')) info.occasion = 'Holiday Celebration';
    else if (text.includes('milestone') || text.includes('promotion')) info.occasion = 'Milestone Achievement';
    else if (text.includes('just because') || text.includes('gesture')) info.occasion = 'Gesture of Devotion';

    // Interests & Personality
    if (text.includes('art') || text.includes('gallery') || text.includes('design')) info.interests = 'Fine Arts & Aesthetics';
    if (text.includes('watch') || text.includes('horology') || text.includes('timepiece')) info.interests = 'Haute Horlogerie';
    if (text.includes('fashion') || text.includes('leather') || text.includes('style')) info.interests = 'Haute Couture';
    if (text.includes('fragrance') || text.includes('perfume') || text.includes('scent')) info.interests = 'Niche Artisanal Perfumery';

    // Emotional Goal
    if (text.includes('timeless') || text.includes('commitment') || text.includes('forever')) info.emotionalGoal = 'Timeless Devotion & Commitment';
    else if (text.includes('gratitude') || text.includes('thank')) info.emotionalGoal = 'Profound Gratitude';
    else if (text.includes('awe') || text.includes('impress') || text.includes('wonder')) info.emotionalGoal = 'Breathtaking Delight';
  ...
  async generateGiftMessage(
    ...
    userPrompt?: string,
    mode: 'write' | 'improve' | 'generate' = 'generate'
  ): Promise<string> {
