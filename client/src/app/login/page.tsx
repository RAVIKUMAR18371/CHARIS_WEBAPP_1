...
export default function LoginPage() {
  const [email, setEmail] = useState('client@charis.com');
    ...
    <div className="min-h-[85vh] flex items-center justify-center px-4 py-16 bg-[#0B090A]">
      ...
      >
        ...
        <div className="text-center space-y-2">
          ...
          <h2 className="font-serif text-3xl font-bold text-[#F9F6F0]">Welcome Back</h2>
          <p className="text-xs uppercase tracking-widest text-[#D4AF37]">Enter Your Private Suite</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5 text-xs">
          ...
          <LuxuryButton type="submit" variant="gold" size="lg" disabled={loading} className="w-full">
            {loading ? 'Authenticating...' : 'Sign In To Suite'} <ArrowRight className="w-4 h-4 ml-1" />
          </LuxuryButton>
        </form>

        <div className="text-center pt-4 border-t border-[#D4AF37]/15 text-xs text-[#C5BFB6]">
          Don't have a private account?{' '}
          <Link href="/register" className="text-[#D4AF37] font-serif italic hover:underline">
            Request Access
          </Link>
        </div>
      </motion.div>
